...
# For this example, we would add one worker if it was compatible with the Documenter.jl 
# ```julia
# addprocs(1)
# ```
...
# Due to a bug in the Documenter we will use `JuliaBackend`
eki = CAL.calibrate(
    CAL.WorkerBackend,
    ensemble_size,
    n_iterations,
...
# Once the calibration is complete, we can visualize the results:
...
display(scatter_plot(eki))
display(param_versus_iter_plot(eki))
