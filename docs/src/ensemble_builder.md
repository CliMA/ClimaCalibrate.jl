...
To determine whether a `OutputVar` match with a metadata, `GEnsembleBuilder`
...
`OutputVar` and metadata with [`ShortNameChecker`](@ref ClimaCalibrate.Checker.ShortNameChecker).

Checkers are objects that implement the `AbstractChecker` interface and perform
...
- [`ShortNameChecker`](@ref ClimaCalibrate.Checker.ShortNameChecker): Validates that the short names match between `OutputVar` and metadata
- [`DimNameChecker`](@ref ClimaCalibrate.Checker.DimNameChecker): Ensures dimension names are consistent
- [`UnitsChecker`](@ref ClimaCalibrate.Checker.UnitsChecker): Checks that variable units are compatible
- [`DimUnitsChecker`](@ref ClimaCalibrate.Checker.DimUnitsChecker): Validates units of dimensions
- [`DimValuesChecker`](@ref ClimaCalibrate.Checker.DimValuesChecker): Compares dimension values for compatibility
- [`SequentialIndicesChecker`](@ref ClimaCalibrate.Checker.SequentialIndicesChecker): Ensures temporal data maps to sequential indices

By default, `GEnsembleBuilder` uses a standard set of checkers to validate
compatibility. You can also provide additional checkers using the `checkers`
...
struct TrueChecker <: Checker.AbstractChecker end
...
- include a `verbose` keyword argument for detailed logging,
