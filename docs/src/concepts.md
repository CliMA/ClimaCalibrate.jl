...
3. **Evaluate the observation map.** Your `observation_map(interface,
   iteration)` reads the members' output and returns the **G ensemble
   matrix**: one column per member, each column comparable with
   the observation.
...
- **A prior** for the parameters being calibrated.
...
- **Noise covariance**: how much of the mismatch between model and observations to attribute
