...
## Customizing the diagonal term of `SVDplusDCovariance`

The covariance matrix produced by `SVDplusDCovariance` observation recipe is the
sum of a low rank approximation of the covariance matrix computed from the
samples and a diagonal matrix. The diagonal matrix is described by a diagonal
term, a subtype of [`ObservationRecipe.AbstractDiagonalTerm`](@ref
ClimaCalibrate.ObservationRecipe.AbstractDiagonalTerm). A diagonal term
describes how the matrix should be built from a `SampleCollection`. It is not a
matrix itself.

The keyword constructor builds the diagonal term from `model_error_scale` and
`regularization`. You can pass your own diagonal term as the first positional
argument to [`SVDplusDCovariance(::AbstractDiagonalTerm)`](@ref
...
- [`ModelErrorScaleDiagonal`](@ref
  ...
  ignoring `NaN`s, and the scale is the model error scale of the variable that
  the entry belongs to.
- [`VarianceDiagonal`](@ref
  ClimaCalibrate.ObservationRecipe.VarianceDiagonal): the entries are the
  variance of each row of the sample matrix, taken across the samples and
  ignoring `NaN`s.
...
Diagonal terms can be combined with broadcasting, which produces a
[`BroadcastedDiagonal`](@ref
ClimaCalibrate.ObservationRecipe.BroadcastedDiagonal). Once every term is
computed, the expression is broadcast over the resulting `Diagonal` matrices.
For example, `ModelErrorScaleDiagonal(0.05) .+ ScalarDiagonal(1e-6)` sums two
terms and `2.0 .* VarianceDiagonal()` scales one, but `VarianceDiagonal() .+
1e-6` is an error since adding a scalar to a `Diagonal` matrix does not produce
a diagonal matrix. A `Diagonal` matrix can be mixed in as well.
