...
function ClimaCalibrate.observation_map(
    ...
    for m in 1:ensemble_size
        ...
        try
            ...
            # A member that produced no usable output reads as `NaN`, which is
            # what tells the update to leave it out. Report why: a member whose
            # output is missing and one whose output is unphysical are
            # different problems, and both arrive here
            @warn "Could not read the output of member $m in $member_path" exception =
                e
            G_ensemble[:, m] .= NaN
        end
    end
