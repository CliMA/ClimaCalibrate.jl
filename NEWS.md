...
- `set_worker_loggers(workers)` ignored `workers` and set the logger on every
  worker.
- A `WorkerBackend` could dispatch an ensemble member to a worker that had not
  finished starting. A worker appears in `Distributed.workers()` when it
  registers, which is before ClimaCalibrate loads any code onto it, and
  `calibration_worker_pool` put it straight into the pool from there. With
  workers submitted asynchronously, a member dispatched into that window died
  with `KeyError: ClimaCalibrate not found`. Workers found in `workers()` are
  now initialized before they are pooled, which is also what workers added with
  a plain `addprocs` needed.
- A scheduler that refuses a submission is reported with what it wrote to
  `stderr`. `submit_job` used to interpolate the `ProcessFailedException`,
  which prints the whole environment the job was to be submitted with, burying
  the one line the scheduler wrote.
- `run_iteration(::HPCBackend, ...)` writes the model interface its job scripts
  load. Only `calibrate` wrote it, so running a single iteration by hand, which
  is how a calibration is restarted, gave an ensemble in which every member died
  with `SystemError: opening file .../interface.jld2`. It also takes the
  absolute path of the output directory, as `calibrate` does, since a member's
  job runs on another node.
- An `EnsembleKalmanProcess` file that cannot be read is reported with its path.
  A run killed part way through writing one leaves it truncated, and JLD2
  answers `InvalidDataException: Did not find a Superblock` without saying which
  file it means.
- `SlurmConfig` directives no longer have underscores rewritten in their
  *values*: `:partition => "gpu_debug"` used to emit `--partition=gpu-debug`.
  ...
  that `sbatch` rejects.
- PBS reported queued and held jobs as `RUNNING`, so `ispending` was
  unreachable, and the `qstat` states `H`, `W`, `T`, `M`, `S`, `B`, and `X`
...
- On Derecho, `qstat` is NCAR's caching wrapper (qstat-cache), which answers
  ...
  last refresh, and a stale state for a job that had since finished, so status
  fell back to "assume running" on every poll, filled the log with the
  submission environment, and let `job_status` report a finished job as
  `PENDING`. The poller now sets `QSCACHE_BYPASS=true` so the scheduler itself
  answers, and the fallback warning reports the qstat error rather than the
  environment.
...
- `Checker.check(::SignChecker, ...)` required the `data` keyword argument that
  ...
  contained `NaN`s.
- A `WorkerBackend` calibration no longer aborts with
  `No active worker available in pool` after a worker dies running an ensemble
  member. `run_iteration` returned the dead worker to the pool that
  `Distributed` had just removed it from, and the next draw from that pool
  threw instead of waiting for the workers still joining. Dead workers now stay
  out of the pool, and the dispatch loop no longer blocks on a pool whose
  workers are all checked out, so a checked-out worker that dies cannot stall
  an iteration.
...
- The surface-fluxes example calibrates `coefficient_a_m_businger` alone with
  unscented Kalman inversion, against a synthetic observation that carries a 1%
  error, from a prior centered on 3.0 while the observation was generated with
  4.7. One parameter takes a stencil of three members, so the calibration costs
  45 forward model runs over 15 iterations, which the docs build runs to
...
- The forward model adds a 1% error of its own, standing in for the internal
  ...
  Kalman method does with it. The example also calibrated
