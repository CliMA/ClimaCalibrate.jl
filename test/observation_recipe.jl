...
@testset "Stacked samples" begin
    ...
    stacked_samples = ext._stacked_samples(vars, sample_dates)

    @test length(stacked_samples) == length(sample_dates)

    for ((start_date, end_date), stacked_sample) in
        zip(sample_dates, stacked_samples)
        window_dates =
...
@testset "SVDplusDCovariance" begin
    ...
    @test svd_plus_d_covar.diag_cov ==
          Diagonal([Float32(0.001) for _ in 1:sample_size])
    ...
    @test svd_plus_d_covar.diag_cov == Diagonal(
        Float32.(
            vec(
                mean(
                    hcat(ext._stacked_samples((var,), sample_dates)...),
                    dims = 2,
                ) .^ 2,
            ),
        ),
    ...
    @test svd_plus_d_covar.diag_cov ==
          Diagonal([Float32(regularization) for _ in 1:sample_size]) + Diagonal(
        Float32.(
            vec(
    ...
    # Test float type
    covar_estimator =
        ObservationRecipe.SVDplusDCovariance(sample_dates, float_type = Float32)
    svd_plus_d_covar = ObservationRecipe.covariance(
        covar_estimator,
        var,
        Dates.DateTime(2007, 12),
...
@testset "Seasonal diagonal covariance" begin
    ...
    # Check float type
    covar_estimator =
        ObservationRecipe.SeasonalDiagonalCovariance(float_type = Float32)
...
@testset "Observation" begin
    ...
    # We already test the covariance structure, so we test if the flattened
