...
@testset "ScalarCovariance" begin
    ...
    nan_sample_collection = SampleBuilder.build_samples_by_times(
        nan_var,
        [(sample_start_date, sample_end_date)];
        FT = Float64,
    )
    ...
    covar_estimator = ObservationRecipe.ScalarCovariance(scalar = 10.0)
    scalar_covar =
        ObservationRecipe.covariance(covar_estimator, nan_sample_collection)
...
@testset "Covariance with latitudes that differ across samples" begin
    ...
    sample_collection = SampleBuilder.build_samples(
        reshape([var_with_lat(lats1), var_with_lat(lats2)], 1, :);
        ignore_dims = ("lat",),
    )
    ...
    for covar_estimator in (
        ...
        @test_throws r"latitudes of sample 2" ObservationRecipe.covariance(
            covar_estimator,
            sample_collection,
        )
    end

    # Without latitude weighting, the latitudes are never used
    @test ObservationRecipe.covariance(
        ObservationRecipe.ScalarCovariance(),
        sample_collection,
    ) isa Diagonal

    # Ignoring the latitude dimension does not prevent latitude weighting from
    # being used
    same_lat_collection = SampleBuilder.build_samples(
        reshape([var_with_lat(lats1), var_with_lat(lats1)], 1, :);
        ignore_dims = ("lat",),
    )
    @test ObservationRecipe.covariance(
        ObservationRecipe.ScalarCovariance(use_latitude_weights = true),
...
@testset "SVDplusDCovariance" begin
    ...
    @test svd_plus_d_covar.diag_cov == Diagonal(
        vec(mean(SampleBuilder.get_samples(sample_collection), dims = 2) .^ 2),
    )
    ...
    svd_plus_d_covar_rank0 =
        ObservationRecipe.covariance(covar_estimator_rank0, sample_collection)
    ...
    # Two or more samples are required
    one_sample_collection = SampleBuilder.build_samples_by_times(
        [var],
    ...
    @test_logs (:warn, r"80 non-positive or NaN diagonal entries") ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(),
        sample_collection,
    )

    # Warn when any entries of the diagonal matrix is 0
    single_zero_row_sample_collection = deepcopy(sample_collection)
    single_zero_row_sample_collection.samples[1, :] .= 0.0
    @test_logs (:warn, r"1 non-positive or NaN diagonal entries") ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(model_error_scale = 0.05),
        single_zero_row_sample_collection,
    )

    # Check no warning is shown when regularization is used
    @test_logs ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(
...
@testset "Quantile regularization" begin
    ...
    function compute_model_error_scale_vec(sample_collection, model_error_scale)
        return vec(
            model_error_scale .*
            mean(SampleBuilder.get_samples(sample_collection), dims = 2),
        ) .^ 2
    ...
    model_error_scale_vec =
        compute_model_error_scale_vec(sample_collection, model_error_scale)
    five_percentile =
    ...
    sample_collection12 = SampleBuilder.build_samples_by_times(
        [var, var2],
        sample_date_ranges;
        FT = Float64,
    )
    ...
    i = 0
    for v in (var, var2)
        sample_collection_v = SampleBuilder.build_samples_by_times(
            [v],
            sample_date_ranges;
            FT = Float64,
        )
    ...
    # Test with small values along the diagonal
    scaled_var = var * 1e-5
    scaled_sample_collection = SampleBuilder.build_samples_by_times(
        [scaled_var],
        sample_date_ranges;
        FT = Float32,
    )
...
@testset "SVDplusDCovariance with diagonal term" begin
    ...
    @test covar_estimator_legacy.diagonal isa
          ObservationRecipe.BroadcastedDiagonal
    ...
    # use_weighted_samples_for_diagonal controls whether the diagonal term is
    ...
    # QuantileRegularization lowers to a QuantileDiagonal of the model error
    # scale term
    @test ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(
            model_error_scale = 1.0,
        ...
        sample_collection,
    ).diag_cov == ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(
            mes_term .+ ObservationRecipe.QuantileDiagonal(0.5, mes_term),
        ),
        sample_collection,
    ).diag_cov

    # A zero diagonal term warns like the keyword defaults do
    @test_logs (:warn, r"4 non-positive or NaN diagonal entries") ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(
    ...
    lat_weights = ext._flat_lat_weights(
        ext._metadata_of_first_sample(lat_sample_collection);
        min_cosd_lat = 0.2,
    )
    @test diag_cov_weighted ≈ Diagonal(diag_cov_unweighted.diag .* lat_weights)
    var_term = ObservationRecipe.VarianceDiagonal()
    @test ObservationRecipe.covariance(
        ObservationRecipe.SVDplusDCovariance(
        ...
        ObservationRecipe.covariance(
            ObservationRecipe.SVDplusDCovariance(var_term),
            lat_sample_collection,
        ).diag_cov.diag .* lat_weights .+ 1.0,
    )
end
...
@testset "Seasonal diagonal covariance" begin
    ...
    seasonal_covariance =
        ObservationRecipe.covariance(covar_estimator, lat_sample_collection)
    ...
    one_sample_collection =
        SampleBuilder.build_samples_by_times([var], [first(sample_date_ranges)])
    ...
    monthly_time = ClimaAnalysis.Utils.date_to_time.(
        Dates.DateTime(2007, 12),
    ...
    monthly_var =
        ...
        one_to_n_data() |>
        initialize
    duplicate_season_collection = SampleBuilder.build_samples_by_times(
        [monthly_var],
        [
    ...
    multiple_year_collection = SampleBuilder.build_samples_by_times(
        [seasonal_var],
        [
    ...
    @test_throws ErrorException ObservationRecipe.covariance(
        covar_estimator,
        multiple_year_collection,
    )

    # Order of seasons differs across samples (one column is [DJF, MAM] and the
    # other is [MAM, JJA])
    misaligned_season_collection = SampleBuilder.build_samples_by_times(
        [seasonal_var],
        [
    ...
    @test_throws ErrorException ObservationRecipe.covariance(
        covar_estimator,
        misaligned_season_collection,
    )

    # Check that entries of zero in the diagonal of the covariance are caught
    constant_var = deepcopy(var)
    constant_var.data .= 1.0
    constant_samples = SampleBuilder.build_samples_by_times(
        [constant_var],
        sample_date_ranges;
        FT = Float64,
    )
    @test_throws r"short name hi" ObservationRecipe.covariance(
        ObservationRecipe.SeasonalDiagonalCovariance(),
        constant_samples,
    )
end
...
@testset "Observation with no time dimension" begin
    ...
    sample_collection = SampleBuilder.build_samples([var, neg_var])
    obs = ObservationRecipe.observation(covar_estimator, sample_collection, 1)
...
@testset "Get information about metadata from nth iteration" begin
    ...
    obs2 = ObservationRecipe.observation(
        covar_estimator,
        SampleBuilder.build_samples_by_times([time_var], sample_date_ranges),
        1,
    )
