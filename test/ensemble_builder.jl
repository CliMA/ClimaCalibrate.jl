...
@testset "Use GEnsembleBuilder for a fake calibration" begin
    ...
    g_ens_builder = EnsembleBuilder.GEnsembleBuilder(eki, Float64)
    @test size(g_ens_builder.g_ens) == (16, EKP.get_N_ens(eki))
...
@testset "Error handling when constructing GEnsembleBuilder" begin
    ...
    time_var =
        TemplateVar() |>
        add_dim("time", time, units = "s") |>
        add_attribs(start_date = "2007-12-1", blah = "blah2") |>
        one_to_n_data() |>
        initialize
    covar_estimator = ObservationRecipe.SeasonalDiagonalCovariance()
    obs_vec = [
    ...
    @test_throws ErrorException EnsembleBuilder.GEnsembleBuilder(eki, Float32)
end
