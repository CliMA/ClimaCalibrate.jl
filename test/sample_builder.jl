...
@testset "Construct single sample" begin
    ...
    # Warn when an OutputVar contributes no data (all NaNs are dropped when
    # flattening)
    all_nan_var = deepcopy(var)
    all_nan_var.data .= NaN
    @test_logs (:warn, r"flattened length of zero") SampleBuilder.build_samples(
        to_row_vec([all_nan_var, all_nan_var]),
    )
...
@testset "Getters" begin
    ...
    @test SampleBuilder.get_metadata(sample_collection) ===
          sample_collection.metadata

    # The second column of the matrix of samples and metadata corresponds to
    # the second sample
...
@testset "Samples by windowing using time ranges" begin
    ...
    dates = ClimaAnalysis.Utils.date_to_time.(
        Dates.DateTime(2008),
        [Dates.DateTime(2008, i) for i in 1:12],
    )
    pr_var_abs =
...
@testset "Reconstruct" begin
    ...
    for (reconstructed_col, vars) in
        ((reconstructed_col1, vars1), (reconstructed_col2, vars2))
