...
@testset "Format slurm time" begin
    @test ClimaCalibrate.BetterBackend.format_slurm_time(1) == "00:01:00"
    @test ClimaCalibrate.BetterBackend.format_slurm_time(60) == "01:00:00"
    @test ClimaCalibrate.BetterBackend.format_slurm_time(90) == "01:30:00"
    @test ClimaCalibrate.BetterBackend.format_slurm_time(1440) == "1-00:00:00"
end
...
@testset "Generate slurm script" begin
    ...
    sbatch_string = ClimaCalibrate.BetterBackend.make_job_script(
        backend,
        job_body;
        job_name,
        output,
    ...
    #SBATCH --time=$(ClimaCalibrate.BetterBackend.format_slurm_time(time_limit))

    $module_str
    export CLIMACOMMS_DEVICE="CUDA"
    export CLIMACOMMS_CONTEXT="MPI"

    srun --output=log.txt --open-mode=append julia --project=$experiment_dir -e 'sleep(30)
    '
...
@testset "Submit slurm job" begin
    ...
    @testset "Submit a slurm job and cancel it" begin
        ...
        # Cancelling is quicker than waiting for a job to complete so we wait only
        # for one minute
        wait_for(job, 60)
        @test ClimaCalibrate.BetterBackend.job_status(job) == :COMPLETED
        @test ClimaCalibrate.BetterBackend.iscompleted(job)
