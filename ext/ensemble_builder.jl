...
struct GEnsembleBuilder{
    ...
    T <: Tuple{Vararg{AbstractMatcher}},
}
    ...
    """A list of matchers used to match the OutputVar to the appropriate
    metadata"""
    matchers::T
end
...
function _try_fill_g_ens_col_with_var!(
    ...
    _is_compatible_with_metadata(g_ens_builder.matchers, var, metadata) ||
        return false
...
function _is_compatible_with_metadata(
    ...
    return all(
        EnsembleBuilder.match(matcher, var, metadata) for matcher in matchers
    )
end
