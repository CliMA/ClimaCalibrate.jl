...
function _check_dates_in_var(vars, start_date, end_date)
    for var in vars
        start_date in ClimaAnalysis.dates(var) || error(
            "$start_date is not a date in OutputVar with short name $(get(var.attributes, "short_name", nothing))",
        )
        end_date in ClimaAnalysis.dates(var) || error(
            "$end_date is not a date in OutputVar with short name $(get(var.attributes, "short_name", nothing))",
        )
    end
    return nothing
end
