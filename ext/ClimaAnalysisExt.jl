        ...
        if covar_estimator.use_latitude_weights
            flattened_lat_weights =
                ClimaAnalysis.flatten(
                    _lat_weights_var(seasonal_variance_var),
                ).data
...
"""
    ObservationRecipe.change_data_type(var::OutputVar, float_type)
...
Return an OutputVar where each data value corresponds to
`(1 / max(cosd(lat), min_cos_lat))` at its coordinate.
"""
function _lat_weights_var(var::OutputVar; min_cos_lat = 0.1)
    ...
    # Because ClimaAnalysis units system does not know about degrees_north and
    # degrees_east, we will check for units with a list of units instead
    deg_unit_names = [
        "degrees",
        "degree",
        "deg",
    ...
    lat_weights = one(FT) ./ max.(cosd.(lats), FT(min_cos_lat))
