...
function submit_job(backend::SlurmBackend, job_script::String)
    ...
    mktemp(output_dir) do sbatch_filepath, io
        ...
        if !iszero(exit_code) || isnothing(job_id)
            error("sbatch exited with $exit_code and returned no job id: \
                  $(isempty(stderr_text) ? output : stderr_text)")
        end
        job_info = JobInfo(backend, parse(Int, job_id.match), job_script)
