...
function make_job_script(
    backend::SlurmBackend,
    job_body; # TODO: This is hardcoded to only work for job_body that run a single julia command
    job_name = "slurm_job",
    output = "output.txt",
    log = "log.txt", # TODO: Should this keyword argument exists?
)
    # TODO: module_load_string was problematic since it tracks climacommon version
    module_load_str = module_load_string(backend)
    (; hpc_kwargs) = backend
    slurm_directives = _generate_sbatch_directives(hpc_kwargs)
    # TODO: All of these should be validated in hpc_kwargs
    ...
    # TODO: I don't know why this exists
    mpiexec_string =
    ...
    exit 0
    """
    # TODO: mpiexec_string should only be inserted if it found julia (insert
    # before)
    # TODO: job_body should start with julia or not idk
    return slurm_script
end
...
"""
    submit_job(
        backend::SlurmBackend,
        job_script::String; # TODO: Maybe use a specific type here like SlurmScript
        output_dir = pwd(), # TODO: I am not sure if this is a reasonable default
    )
...
The keyword argument `output_dir` is the directory where a temporary job script
file is created.
"""
function submit_job(
    backend::SlurmBackend,
...
"""
    kill_job(::SlurmBackend, job::JobInfo)

Kill `job` by running `scancel`.
"""
function kill_job(::SlurmBackend, job::JobInfo)
    (; id) = job
    try
        run(`scancel $id`)
