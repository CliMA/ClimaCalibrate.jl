...
[`compute_diagonal`](@ref) builds the matrix once the samples are available.
...
# Implement the styles needed for broadcasting with scalars, matrices, and other
# diagonal terms. Broadcasting under this style only records the expression.
