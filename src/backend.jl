...
before all workers have connected. Add workers with [`add_workers`](@ref) on a
...
function cancel_jobs_at_exit(backend::HPCBackend)
    ...
    # once with the same backend. Registering the hook twice would cancel all
    # job twice
...
This returns a type, not a backend that `calibrate` accepts. Construct a backend
by calling the type. When it is an `HPCBackend`, pass a config with
`backend_type()(; directives, modules, env_vars)`; a [`JuliaBackend`](@ref) takes
none of those and is built with `JuliaBackend()`.
"""
...
function get_backend()
    Base.depwarn(
        ...
        renamed to `backend_type` to say so. Construct a backend by calling the \
        type: `backend_type()(; directives, modules, env_vars)` for an \
        `HPCBackend`, or `JuliaBackend()` off a cluster.",
        :get_backend,
    )
    return backend_type()
end
...
Run `cmd`, returning `(stdout, stderr, exit_code)` with both streams stripped of
surrounding whitespace.

Submission commands carry the whole environment (`sbatch`/`qsub` are handed one
with the scheduler's own variables removed), and a `ProcessFailedException`
prints that environment. Reporting what the command wrote to `stderr` instead
puts the scheduler's own diagnostic, which is one line, where a reader will see
it.
"""
function _run_capturing_output(cmd::Base.AbstractCmd)
    out = IOBuffer()
    err = IOBuffer()
    process = run(
        pipeline(ignorestatus(cmd); stdout = out, stderr = err);
        wait = true,
    )
