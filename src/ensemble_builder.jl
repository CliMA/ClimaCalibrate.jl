...
Return `true` once all entries of the G ensemble matrix have been filled.
...
"""
    ranges_by_short_name(builder, short_name)

Return the row ranges of the G ensemble matrix that `short_name` occupies, as a
vector with one range per observation carrying that short name.
...
"""
    metadata_by_short_name(builder, short_name)

Return the observation metadata the builder matches `short_name` against, as a
vector with one entry per observation carrying that short name.
