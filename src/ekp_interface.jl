...
"""
    observation_map_and_update!(ekp, output_dir, iteration, prior, ctx)

Compute the observation map and update the given EKP object.
"""
function observation_map_and_update!(ekp, output_dir, iteration, prior, ctx)
    g_ensemble = ClimaCalibrate.observation_map(ctx, iteration)
    ...
    try
        ClimaCalibrate.analyze_iteration(ctx, ekp, g_ensemble, prior, output_dir, iteration)
    catch ret_code
