...
A run that was killed while writing this file leaves it truncated, which JLD2
reports as a missing superblock without naming the file, so the error says which
one it is and what to do about it.
"""
function load_ekp_struct(output_dir, iteration)
    file = ekp_path(output_dir, iteration)
    return try
        JLD2.load_object(file)
    catch e
        error(
            "Could not read the EnsembleKalmanProcess at $file: \
            $(sprint(showerror, e)). A run that was killed part way through \
            writing it leaves it unreadable; delete iteration $iteration, or \
            calibrate into an output directory of its own.",
        )
    end
end
...
Return the samples of all observations in `ekp`'s series, and the shapes of
