...
Returns a vector of length `n_variables`. These are shares of the total: under
the noise model they sum to the same value that [`compute_structured_energy`](@ref)
reports, which is approximately one, so each entry is well below one on its own.
A variable holding most of the total is the one carrying the structure, and a
variable near zero is one the leading eigenvectors barely touch. Compare
variables against each other rather than against one.
...
- `structured_energy_by_variable`: How that energy is split between variables
  `[-]`. Under the noise model the entries sum to `structured_energy`, so compare
  them against each other rather than against one.
