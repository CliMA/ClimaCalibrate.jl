...
indicate mismatch beyond what the structured noise explains.
"""
function compute_structured_energy(projections)
    z = vec(sum(projections, dims = 2))
...
"""
    compute_normalized_projections(diff, eigvectors, eigvalues, metadata_vec)
...
Returns a `Matrix` of shape `(n_eigenvectors, n_variables)`. Each element of
`metadata_vec` must have a `range` field giving the observation indices for
that variable. Pass the result to `compute_structured_energy` or
`compute_structured_energy_by_variable` for scalar summaries.
"""
function compute_normalized_projections(
    diff,
    eigvectors,
    eigvalues,
    metadata_vec,
    ...
    n_variables = length(metadata_vec)
    projections = zeros(n_eigenvectors, n_variables)
    for (v, metadata) in enumerate(metadata_vec)
        r_v = metadata.range
...
Requires ClimaAnalysis to be loaded (for `EnsembleBuilder.GEnsembleBuilder`).
"""
function analyze_residual(ekp, iter; n_eigenvectors = 3)
    ...
    # TODO: Don't use g_ens_builder just to extract the metadata ranges
    g_ens_builder = EnsembleBuilder.GEnsembleBuilder(ekp)
    metadata_vec = g_ens_builder.metadata_vec
    projections = compute_normalized_projections(
        diff,
        eigvectors,
        eigvalues,
        metadata_vec,
    ...
    return (;
        ...
        residual_norm_by_variable = map(m -> norm(diff[m.range]), metadata_vec),
    )
end
