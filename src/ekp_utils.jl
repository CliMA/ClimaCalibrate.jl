...
dropped and a warning is emitted. `batch_size` larger than `n_samples` leaves no
minibatch at all, which is an error: EKP divides by the number of minibatches in
an epoch.
...
function minibatcher_over_samples(n_samples::Int, batch_size::Int)
    ...
    batch_size <= n_samples || throw(
        ArgumentError(
            "Batch size ($batch_size) is larger than the number of samples \
            ($n_samples), which leaves no minibatch to draw from",
        ),
    )
...
Return `true` if `minibatcher` gives all epochs the same minibatches, so that
