...
The loop is the same for all backends: write the ensemble members' parameters,
...
also check that the experiment and model interface files their job scripts load
exist, and register the exit hook that cancels submitted jobs.
"""
...
function run_iteration(
    ...
    # A member's job runs in a process of its own, on another node, so it takes
    # absolute paths and loads the interface from a file. Writing that file here
    # rather than in `initialize_backend!` is what lets an iteration be run on
    # its own, which is how a calibration is restarted by hand
    output_dir = abspath(output_dir)
...
On PBS that means running `qstat`, uncached, so the interval between polls grows
with the number of members still running.
    ...
    try
        while length(completed_jobs) < length(jobs)
            ...
            if length(completed_jobs) < length(jobs)
                ...
                # One uncached `qstat`/`squeue` per unfinished member per poll,
                # so back off as the ensemble grows rather than letting the
                # query rate grow with it
                outstanding = length(jobs) - length(completed_jobs)
                sleep(clamp(0.5 * outstanding, 5, 30))
            end
        end
    catch e
        Backend.cancel_job.(
            filter(j -> !isnothing(j) && !Backend.job_finished(j), jobs),
        )
    ...
    @sync while !isempty(work_to_do)
        ...
        if worker !== nothing
            ...
            @async try
                ...
                # A worker that died running the member (the
                # `ProcessExitedException` caught above) has already left the
            ...
            # Wake as soon as a worker is queued, else recheck the stall clock
            timedwait(() -> isready(worker_pool), 10.0)
        end
    end
...
"""
    take_live_worker!(pool::Distributed.WorkerPool)

Draw a live worker from `pool` without blocking, or return `nothing` if none is
queued.

`take!(::WorkerPool)` does not fit the dispatch loop in [`run_iteration`](@ref).
It blocks while every pooled worker is checked out, and a checked-out worker
that dies is not returned to the pool, so the block would never end. And it
prunes dead ids lazily, on the draw that finds them, throwing
`"No active worker available in pool"` when the pruned id was the pool's last
instead of reporting an empty pool. Dead ids do reach the channel: the
`:deregister` hook removes a worker from the pool's `Set` but not from the copy
queued in its `Channel`.
"""
function take_live_worker!(pool::Distributed.WorkerPool)
    while isready(pool)
        id = take!(pool.channel)
        id in Distributed.procs() && return id
